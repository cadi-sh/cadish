...
**1. Install the controller** — one command, nothing to clone:

```bash
kubectl apply -f https://cadi.sh/install/ingress.yaml
kubectl rollout status deploy/cadish-ingress -n cadish
```

That installs the `cadish` `IngressClass`, RBAC, the base ConfigMap, the controller
Deployment (image `ghcr.io/cadi-sh/cadish`), and a `LoadBalancer` Service — all in the
`cadish` namespace. Prefer Helm? It is published as an OCI chart, also no clone:

```bash
helm install cadish oci://ghcr.io/cadi-sh/charts/cadish --version 0.2.2 \
  --namespace cadish --create-namespace \
  --set controller.enabled=true \
  --set controller.publishService=cadish/cadish-ingress
```

**2. Create a tiny backend** — an `echo` Deployment + Service in its own namespace:

```bash
kubectl create namespace demo
kubectl create deployment echo --image=ealen/echo-server -n demo
kubectl expose deployment echo --port=80 -n demo
```
...
curl -H 'Host: echo.example.com' "http://${LB}/"
```

You should get the `echo` pod's request dump. The Ingress `ADDRESS` column
...
Two no-clone paths (recommended) and a from-source path. All install the same stack —
IngressClass `cadish`, RBAC, base ConfigMap, controller Deployment (2 replicas), and a
`LoadBalancer` Service — into the `cadish` namespace, pulling the public image
`ghcr.io/cadi-sh/cadish`.

### Hosted manifest (no clone)

```bash
kubectl apply -f https://cadi.sh/install/ingress.yaml          # latest stable (pinned)
kubectl apply -f https://cadi.sh/install/ingress-0.2.2.yaml    # or pin a version
```

A single combined manifest, generated from the `deploy/k8s/` files below — the same
`kubectl apply -f <URL>` flow other ingress controllers ship.

### Helm (OCI chart, no clone)
...
The chart is published to GHCR as an OCI artifact, so there is no `helm repo add` and no
clone. `controller.enabled=true` switches the chart from the plain `run` Deployment to the
Ingress-controller stack.

### From source (clone)

If you have the repo checked out (to customize the manifests or chart):

```bash
kubectl apply -f deploy/k8s/namespace.yaml
kubectl apply -f deploy/k8s/rbac-controller.yaml
kubectl apply -f deploy/k8s/ingress-controller.yaml
# or: helm install cadish deploy/helm/cadish --namespace cadish --create-namespace \
#       --set controller.enabled=true --set controller.publishService=cadish/cadish-ingress
```

> The controller is **not** part of `kubectl apply -k deploy/k8s` (that overlay deploys
> the standalone `run` server) — apply the three controller manifests explicitly, or use
> the hosted manifest / Helm above.
