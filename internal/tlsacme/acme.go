...
type autocertSource struct {
	...
	// memoizes names already served so the hot path skips the limiter and the disk probe.
...
func newAutocertSource(cacheDir, email string, policy autocert.HostPolicy, directoryURL string, httpClient *http.Client) *autocertSource {
	...
	return &autocertSource{mgr: m, cache: cache, limiter: newIssuanceLimiter(time.Now)}
}
...
// handshake, a name already served this run, and a name already in the on-disk cache all pass
// straight through to autocert. Only a name that would trigger a brand-new ACME order consults
// the rate limiter; when the budget is exhausted the handshake fails for that one new name
// (existing certs keep serving) so a random-SNI flood cannot exhaust the ACME account.
func (a *autocertSource) GetCertificate(hello *tls.ClientHelloInfo) (*tls.Certificate, error) {
	...
	if _, ok := a.issued.Load(name); ok {
		return a.mgr.GetCertificate(hello) // already served this run (autocert's in-memory cache)
	}
	if a.HasIssuedCert(name) {
		// Already issued on disk (e.g. across a restart) — not a new order; memoize + serve.
		a.issued.Store(name, struct{}{})
		return a.mgr.GetCertificate(hello)
	}
	// A brand-new name → a new ACME order. Gate it on the new-cert budget.
	if !a.limiter.allow() {
